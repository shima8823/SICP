...
(define (remove-index index sequence)
	(let ((len (length sequence)))
		(define (iter prev seq)
			(cond ((null? seq) '())
				  ((= (- len index) (length seq))
				  	(if (null? prev)
						(cdr seq)
						(append prev (cdr seq))))
				  (else (iter (append prev (list (car seq))) (cdr seq)))))
		(iter '() sequence)))

; simple 別解 https://sicp.iijlab.net/solution/ex4.3.html#ex4.50
				...
				(if (null? choices)
					...
					(let ((index (random (length choices))))
						((list-ref choices index)
							env
							succeed
							(lambda () (try-next (remove-index index choices)))))))
			(try-next cprocs))))
...
(define (require p) (if (not p) (ramb)))
