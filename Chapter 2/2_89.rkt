...
合っているが絶対にselector, constructor, mul procを変えるべきではなく、
密に適している表現から疎(多項式はほとんど疎)に適している表現に変換するだけの手続きを作った方が良かった。
前にもこんな問題があり(確か記号微分)内部構造を変えてしまったので次は表現の変換だけをしたい。
...
(define (install-polynomial-package)
	...
	(define (make-term order coeff) (list coeff))
	...
	(define (cons-zero term count)
		; (display term)(newline)
		(if (= count 0) term
			(cons-zero (append term '(0)) (- count 1))))
	(define (mul-terms L1 L2)
		(if (empty-termlist? L1)
			...
			(let ((result
					...
					(max-order (+ (order (first-term L1)) (order (first-term L2)))))
				(if (< (length result) max-order)
					(cons-zero result (- max-order (length result)))
					result))))
	(define (mul-term-by-all-terms t1 L)
		(if (empty-termlist? L)
			(the-empty-termlist)
			(let ((t2 (first-term L)))
				(let ((sum-order (+ (order t1) (order t2))))
					(let 
						((mul-term
							(adjoin-term
								(make-term
						...
						(if (< (length mul-term) sum-order)
							(cons-zero mul-term (- sum-order (length mul-term)))
							mul-term))))))
...
; (define sample1 (make-polynomial 'x '(1 2 0 3 -2 -5)))
(define sample1 (make-polynomial 'x '(5 3 7)))
(define sample2 (make-polynomial 'x '(2 0 0)))
(define sample3 (make-polynomial 'x '(2 0 0 0)))
sample1

(add sample1 sample2) ; (7 3 7)
(add sample1 sample3) ; (2 5 3 7)
(add sample1 sample1) ; (10 6 14)
(mul sample1 sample2) ; (10 6 14 0 0)
(mul sample2 sample1) ; (10 6 14 0 0)
(mul sample1 sample3) ; (10 6 14 0 0 0)
(mul sample3 sample1) ; (10 6 14 0 0 0)
