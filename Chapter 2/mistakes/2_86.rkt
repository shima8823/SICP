...
複素数の実部、虚部、絶対値、偏角に
通常の数値や有理数や、これからシステムに追加するその他の数値を使えるようにしたいとする。
->使えるというのは複素数に演算できるようにすること？
->実部を通常の数値、有理数。虚部に通常の数値と有理数ということ？

add-complexやreal-partで定義されてる基本演算をgeneric演算に置き換える
sin cosもcomplex packageで使われている点から、それぞれの型定義する必要がある
